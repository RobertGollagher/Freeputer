#!/usr/bin/ruby
# ============================================================================
VERSION = "flx.rb Freelang cross compiler version 1.0.0.0 for FVM 1.0"
# ============================================================================
# Program:    flx.rb
# Copyright © Robert Gollagher 2015
# Author :    Robert Gollagher   robert.gollagher@freeputer.net
# Created:    20150329
# Updated:    20150807:2302
...
ILIT = 1
IHIGHEST_COMPLEX_OPCODE = 37 # Higher opcodes are simple not complex instrs
                             #   (or illegal if not > ILOWEST_SIMPLE_OPCODE)
ILOWEST_SIMPLE_OPCODE = 145  # This and higher opcodes are simple instrs
...
# Some instruction ranges
addrInstrs = $iSet.find_index("call")..$iSet.find_index("putorb")
  ...
  (/(\A[A-z_][0-9\?-z]*{[\.\#\$:]{0,2}[A-z_][0-9\?-z]*}\z)/ =~ token)
    ...
    # ========================================================================
      ...
      # ----------------------------------------------------------------------
        ...
        if (expectingAddr and not /#{'(\A:+' + WDPTN + '+\z)'}/ =~ token) then
          # We were expecting an address, and we already know token is not a
          #   numeric literal, so the only legal token here is a label
          #   reference (starts with :), otherwise we have a syntax error
          $syntaxError = true
          printf("%d Not a valid label reference: %s\n", lineNum, token)
          printf("Label declarations must end with :\n")
          printf("Label references must start with :\n")
          break
        end
        # --------------------------------------------------------------------
