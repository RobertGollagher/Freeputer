...
=======
                              ...
                              This Edition:
                     ARM Assembly Language for Linux
                           using GNU Assembler
...
TODO NEXT:
  ...
  3. Change implemention to ARM (this is currently still x86 code).
...
.macro in failureLabel # FIXME no x86-64 support here yet when using LD
  .ifeq LINKING_WITH_LD_ON_LINUX # (and also unread chars spill to terminal)
    pushl %ebx
    pushl %ecx
    pushl %edx
    ...
    1:
      ...
      jmp \failureLabel
    2:
  .else # FIXME think about non-blocking here
    .extern getchar
    call getchar
  .endif
.endm
...
.ifeq LINKING_WITH_LD_ON_LINUX
  ...
  int $0x80                       # Linux interrupt for system call
    testl %eax, %eax
    js 1f                         # Write failed for some reason
...
.lcomm writeBuf, WD_BYTES
.lcomm readBuf, WD_BYTES
##############################################################################
.section .text ###############################################################
##############################################################################
