...
Created:    20170729
Updated:    20170923++
Version:    pre-alpha-0.0.5.11+ for FVM 2.0
=======
...
void inline Dec()    { --vB; }
// Immediates
void inline Imm(METADATA x)    { vB = enrange(x); } // bits 31..0 // NO LONGER USED
...
WORD program[] = {
  ...
  IM,3,ADD,IM,5,ADD,IM,0x7fffffff,FROMB,TOR,NOP,RPT|10,HALT
};
void loadProgram() {
...
int runVM() {
  ...
  while(1) {
    ...
    switch(opcode) { // TODO Fix order
      ...
      case JMPG:   JmpG(instr&MEM_MASK); break;
      case JMPL:   JmpL(instr&MEM_MASK); break;
